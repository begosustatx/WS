<?php 
	...
	if (!empty($testua) || !empty($zailtasun) || !empty($gaiarloa) || !empty($eZuzen) || !empty($eOker1) || !empty($eOker2)|| !empty($eOker3)){
		...
		if (empty($testua) || empty($zailtasun) || empty($gaiarloa) || empty($eZuzen) || empty($eOker1) || empty($eOker2)|| empty($eOker3)){	
			echo "Galdera berria sartzeko derrigorrezko eremu guztiak bete, zure galdera ez da gehitu hauek dira dagoeneko zeuden galderak:";
		}	
		else {
			
			if(strlen($testua)<9){
				echo "Galderaren testu motzegia zure galdera ez da gehitu hauek dira dagoeneko zeuden galderak:";
			}  
			else{		
